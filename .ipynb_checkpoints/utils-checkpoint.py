...
class Mixtures():
  ...
  def forecast(self, steps, predict=['mean']):
    ...
    for col in self.medias:
      arima = auto_arima(self.medias[col], start_p=0, d=1, start_q=0,
    ...
    self.pred_mean = pd.DataFrame(
        ...
        columns=self.medias.columns,
        index=range(self.time_periods[-1]+1 ,self.time_periods[-1]+steps+1)
    )

    # SDs
    self.pred_sd = self.desvios.mean().values
...
class Wgan():
  def __init__(self,
    ...
    self.loader = DataLoader(dataset=self.dataset, batch_size=self.batch_size, shuffle=True) # SHUFFLE
